...
public class CompanyStudentReportService {
    ...
    // Company-wise Report
    // =========================
    public List<CompanyReportDto> getCompanyWiseReport(String token) {
        ...
        for (var company : companies) {
            ...
            for (var job : companyJobs) {
                ...
                if (hiredForJob > 0) {
                    totalPackage += job.getPackageLpa();
                }
            }
    ...
    // Export PDF Report
    // =========================
    public ResponseEntity<byte[]> exportPdfReport(String token) {
        try {
            // 1. Get company report data
            List<CompanyReportDto> reportData = getCompanyWiseReport(token);
